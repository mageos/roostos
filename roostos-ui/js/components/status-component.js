        ...
        if (warningsContainer) {
            ...
            if (warnings.length > 0) {
                ...
                warningsContainer.innerHTML = warnings.map(w => {
                    const isUnrecognizedDevice = w.includes("Unrecognized device");
                    const style = "background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: #ef4444; padding: 12px 16px; border-radius: 6px; margin-bottom: 10px; font-size: 13px; display: flex; align-items: center; gap: 8px;" + (isUnrecognizedDevice ? " cursor: pointer; transition: background 0.2s;" : "");
                    const hoverAttr = isUnrecognizedDevice ? ' onmouseover="this.style.background=\'rgba(239, 68, 68, 0.15)\'" onmouseout="this.style.background=\'rgba(239, 68, 68, 0.1)\'"' : '';
                    const clickAttr = isUnrecognizedDevice ? ' onclick="window.switchView(\'devices\')"' : '';
                    
                    return `
                        <div style="${style}"${hoverAttr}${clickAttr}>
                            ...
                            ${isUnrecognizedDevice ? `<span style="margin-left: auto; font-size: 11px; text-decoration: underline; opacity: 0.8;">Manage &rarr;</span>` : ""}
                        </div>
                    `;
                }).join("");
            } else {
